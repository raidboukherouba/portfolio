...
import { getCertificationById, updateCertification, Certification } from '../../../services/certificationService';
...
const EditCertification = () => {
  ...
  useEffect(() => {
    const fetchCertification = async () => {
      try {
        ...
        setValue('title', certification.title);
        setValue('issuer', certification.issuer || '');
        setValue('date', certification.date ? new Date(certification.date).toISOString().substr(0, 10) : '');
        setValue('description', certification.description || '');
        setValue('credentialUrl', certification.credentialUrl || '');
        setError('');
      } catch (err) {
  ...
  const handleUpdate = async (data: Certification) => {
    try {
  ...
  if (isLoading) {
    return <div className="text-center py-2 text-sm dark:text-gray-300">Loading certification...</div>;
  }
  ...
  return (
    <div className="mx-auto px-6 py-2 bg-white dark:bg-black">
      <div className="flex justify-between items-center mb-2">
        <h1 className="text-lg font-bold dark:text-white">Edit Certification</h1>
        <Link
